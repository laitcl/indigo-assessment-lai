...
CREATE TABLE research.qa_tests (
  ...
  treatment_date_planted                  DATE, -- There are actually two columns called date_planted; so I'm renaming both of them for specificity
  ...
  sample_tested_by_employee_id            INT, -- Foreign Key to employees table once again
  chemical_treatment_visible              VARCHAR(255), -- identical to .xlsx
  test_date_planted                       DATE, -- There are actually two columns called date_planted; so I'm renaming both of them for specificity
  ...
  FOREIGN KEY (sample_tested_by_employee_id) REFERENCES employees(id),
  FOREIGN KEY (sample_seeds_id) REFERENCES sample_seeds(id)
);

-- ----------------------------
-- Indexes for qa_tests
-- ----------------------------
CREATE INDEX date_received_idx ON research.qa_tests (date_received_at_qa); -- couple examples of single idnexes
CREATE INDEX days_between_treatment_and_planting_idx ON research.qa_tests (days_between_treatment_and_planting);
CREATE INDEX barcode_date_received_idx ON research.qa_tests (irp_ga_sample_barcode, date_received_at_qa); -- example of a composite index since these two might be searched together frequently
...
CREATE TABLE research.colony_forming_units (
  ...
  qa_test_id      varchar(255), -- This is a foreign key reference to the correct qa_test experiment
  PRIMARY KEY (id),
  FOREIGN KEY (qa_test_id) REFERENCES qa_tests(id)
);
