...
class qa_test:
    ...
    def warning_on_row(self, index, warning_message):
        warnings.warn(warning_message.format(self.data['irp_qa_sample_barcode'].iloc[index], self.data['date_received_at_qa'].iloc[index]))

    def ensure_accurate_days_between_treatment_and_planting(self):
        calculated = (self.data['sample_date_planted']-self.data['date_treated']).apply(self.t_interval_to_string)
        actual = (self.data['days_between_treatment_and_planting']).apply(lambda x: float(x.split()[0]) if isinstance(x,str) else x) 
        comparison = ((calculated == actual) | (calculated.isnull() & actual.isnull()))
        for i in range(0, len(comparison)):
            if comparison.iloc[i] == False:
                self.warning_on_row(i, "Data at barcode: {0}, date_received: {1} has mismatching days_between_treatment_and_planting")

    def ensure_accurate_average_cfu(self):
        raw_cfu_columns = ['cfu_seed_1x','cfu_seed_10x','cfu_seed_100x','cfu_seed_1000x']
        # 'average_cfu_per_seed' 
        for i, row in self.data.iterrows():
            ...
            if isinstance(row['average_cfu_per_seed'],float) and (not math.isclose(average, row['average_cfu_per_seed'], rel_tol=1e-2, abs_tol=1.0)):
                if (math.isnan(average) or average == 0) and math.isnan(row['average_cfu_per_seed']):
                    continue
                self.warning_on_row(i, "Data at barcode: {0}, date_received: {1} has mismatching average_cfu_per_seed")
    ...
    def generate_employees_csv(self):
        ...
        sample_employees = self.data[['sample_received_by', 'sample_received_by_employee_manager', 'sample_received_by_employee_team']]
        sample_employees.columns=new_columns
        testing_employees = self.data[['sample_tested_by', 'sample_tested_by_employee_manager', 'sample_tested_by_employee_team']]
        testing_employees.columns=new_columns
        employees_table = sample_employees.append(testing_employees).drop_duplicates()
